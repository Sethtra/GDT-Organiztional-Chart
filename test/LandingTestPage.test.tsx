...
describe("LandingTestPage", () => {
  beforeEach(() => {
    window.localStorage.removeItem("gdt_test_landing_theme");
  ...
  it("switches the isolated preview theme and pairs it with the correct logo", async () => {
    window.localStorage.setItem("gdt_test_landing_theme", "dark");
    const user = userEvent.setup();

    renderLanding();

    const page = document.querySelector(".landing-test-page");
    ...
    expect(window.localStorage.getItem("gdt_test_landing_theme")).toBe("light");
  });
